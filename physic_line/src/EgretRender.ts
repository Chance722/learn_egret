...
class EgretRender {
    public static create(options: Object) {
        ...
        render.container = render.container || options['container'] || egret.MainContext.instance.stage
    ...
    public static run(render: Object, callBack?: Function) {
	...
	private static _createBodyPrimitive(render: any, body: any): any {
		...
		for (var k=body.parts.length>1?1:0; k<body.parts.length; k++) {
			...
			strokeStyle = part.strokeStyle || bodyRender.strokeStyle;
			lineWidth = part.lineWidth || bodyRender.lineWidth;
			let points = [];

			// 如果是圆形 则直接采用画圆的方式 避免产生锯齿
			if (part.circleRadius) {
				if (fillStyle !== undefined) {
					primitive.beginFill(fillStyle)
				}
				primitive.lineStyle(lineWidth, strokeStyle)
				primitive.drawCircle(0, 0, part.circleRadius)
				primitive.endFill()
			} else {
